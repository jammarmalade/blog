...
class template {
	...
	function compile_tpl() {
		if($fp = @fopen($this->tplpath, 'r')) {
			...
			$this->_error('模版不存在',$this->tplpath);
		}
		...
		if(!@$fp = fopen($this->tplcachepath, 'w')) {
			$this->_error('目录不存在或目录不可写',$this->tplcachepath);
		}
		flock($fp, 2);//写入锁定
		fwrite($fp, $tpl);
		fclose($fp);
