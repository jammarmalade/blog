...
(function (factory) {
  ...
  var settings = {
    ...
    options: {
      ...
      disableDragAndDrop: true,    // disable drag and drop event 禁用拖拽
...
//       ],
  ...
  var Editor = function () {
     ...
     * @param {String} sUrl
     */
    this.insertImage = function ($editable, sUrl, filename ,imgwidth) {
	  
      async.createImage(sUrl, filename).then(function ($image) {
	  var width=Math.min($editable.width(), $image.width());
	  if(isUndefined(imgwidth) || imgwidth!=''){
		width=imgwidth;
	  }
        beforeCommand($editable);
        $image.css({
